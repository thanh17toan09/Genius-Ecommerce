...
<?php 
    ...
    */
    class Database1
    {
        ...
        {
            $this->link = mysqli_connect("127.0.0.1","root","","smartphone_3t") or die ();
        ...
        public function get_danhmuc($key)
        {
                $sql = "SELECT * FROM nhanhieu WHERE id_NH = '$key'";
            $result = mysqli_query($this->link,$sql) or die("Lỗi  truy vấn fetchID " .mysqli_error($this->link));
            return mysqli_fetch_assoc($result);
        }

        
        public  function thongtindonhang($table)
        {//lấy ra các danh mục nằm trong thương hiệu
            
            $data = [];
            // _debug($sql);die;    
            $sql = "SELECT transaction.*, users.HoTen AS nameuser, users.Phone AS phone, users.DiaChi AS diachi FROM transaction, users  WHERE transaction.user_id = users.id";      
            $result = mysqli_query($this->link,$sql) or die("Lỗi truy vấn thongtindonhang ---- " .mysqli_error($this->link));
            if( $result)
            {
                while ($num = mysqli_fetch_assoc($result))
                {
        ...
        public  function fetchNAME($table, $namecate)
        {
            
            $sql = "SELECT * FROM {$table} WHERE name = '$namecate'";
            $result = mysqli_query($this->link,$sql) or die("Lỗi  truy vấn fetchID " .mysqli_error($this->link));
            return mysqli_fetch_assoc($result);
        }
        ...
        public  function fetchJone($table)
        {//lấy ra các danh mục nằm trong thương hiệu
        ...
        public  function fetchJoneONE($table)
        {
            
            $data = [];
            // _debug($sql);die;    
            $sql = "SELECT product.*, category.name as namecate, category_child.name as namecate_child, category_child.id_parent as id_parent FROM `product`, `category_child` , `category` WHERE category_child.id = product.id_category_child AND category_child.id_parent = category.id GROUP BY product.id";        
            $result = mysqli_query($this->link,$sql) or die("Lỗi truy vấn fetchJone ---- " .mysqli_error($this->link));
            if( $result)
            {
                while ($num = mysqli_fetch_assoc($result))
